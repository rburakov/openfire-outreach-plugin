...
/**
 * MessageArchive IQ Query Handler
 */
class IQMessageArchiveHandler extends IQHandler implements
        ServerFeaturesProvider {

    private static final Logger Log = LoggerFactory.getLogger(IQMessageArchiveHandler.class);
    private static final String MODULE_NAME = "Message Archive Management Query Handler v2a";
    ...
    public static final String FORCE_RSM = "archive.FORCE_RSM";

    protected ExecutorService executorService;
    protected PacketRouter router;

    IQMessageArchiveHandler() {
        super(MODULE_NAME);
    }
    ...
    protected MessageArchivePersistenceManager getPersistenceManager() {
        return OutreachPlugin.getInstance().getMessageArchivePersistenceManager();
    }
    ...
    public IQ handleIQ( final IQ packet ) throws UnauthorizedException {
        ...
        if ( JiveGlobals.getBooleanProperty(FORCE_RSM, true ) ) {
    ...
    private Collection<ArchivedMessage> retrieveMessages(QueryRequest queryRequest) throws NotFoundException {
        ...
        try
        {
            ...
            ZonedDateTime newDate = nowDate.minusDays(0);
            ...
            Collection <ArchivedMessage> result = getPersistenceManager().findMessages(
                    startDate,
                    endDate,
    ...
    private void sendMessageResult(JID from, QueryRequest queryRequest, ArchivedMessage archivedMessage) {
        ...
        try {
            ...
            //clear body when message was deleted
            if (archivedMessage.getDeleteTime() != null) {
                stanza.getRootElement().element("body").setText("");
            }
        ...
        messagePacket.addExtension(new Result(fwd, NAMESPACE, queryRequest.getQueryid(), archivedMessage.getId().toString(), archivedMessage.getDeleteTime(), archivedMessage.getEditTime()));
